Shader "Custom/My/7/MaskTexture"
{
	Properties
	{
...
#include "Lighting.cginc"
			...
			fixed4 frag(v2f o) :SV_Target
			{
				...
				float3 tangentNormal = UnpackNormal(tex2D(_BumpMap, o.uv));
				tangentNormal.xy *= _BumpScale;
				tangentNormal.z = sqrt(1 - saturate(dot(tangentNormal.xy, tangentNormal.xy)));

				fixed3 albedo = tex2D(_MainTex, o.uv).rgb*_Color.rgb;

				fixed3 ambient = glstate_lightmodel_ambient.xyz * albedo;

				fixed3 diffuse = albedo * _LightColor0.rgb *(dot(tangentNormal, tangentLightDir)*0.5 + 0.5);
